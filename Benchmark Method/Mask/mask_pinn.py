...
class InterpolationMaskedDataset(Dataset):
    ...
    def __init__(self, inputs, targets, mask_rate, is_train=True, base_seed=42):
        # 修改1: 直接接收 inputs 和 targets 列表，而不是一个 source_dataset 对象
    ...
    def __getitem__(self, idx):
        original_inputs = self.inputs[idx]
        targets = self.targets[idx]

        # 修改2: __getitem__ 返回值的结构改变
        # 原因: PINN的损失函数需要原始输入来计算物理损失。
        # 因此，我们需要返回 (插值后的输入, 原始输入, 目标)
        if not self.is_train or self.mask_rate == 0:
            return original_inputs, original_inputs, targets

        inputs_clone = original_inputs.clone()
        L, C = inputs_clone.shape
        ...
        return inputs_clone, original_inputs, targets
...
def train_pinn_transfer(model,
    ...
    for epoch in range(pretrain_epochs):
        total_loss = 0
        for interpolated_inputs, original_inputs, targets in source_loader:
            targets = relu_l1_normalize(targets, dim=-1)
            optimizer.zero_grad()
            outputs = model(interpolated_inputs, steps_to_predict=targets.size(1))
            loss = composite_loss(outputs, targets, original_inputs,
    ...
    for epoch in range(finetune_epochs):
        total_loss = 0
        model.train()
        for interpolated_inputs, original_inputs, targets in target_loader:
            targets = relu_l1_normalize(targets, dim=-1)
            optimizer.zero_grad()
            outputs = model(interpolated_inputs, steps_to_predict=targets.size(1))
            loss = composite_loss(outputs, targets, original_inputs,
...
for fold_num, (train_idx, test_idx) in enumerate(kf.split(subjects), 1):
    for start_day in START_DAYS:
        ...
        pretrain_dataset = InterpolationMaskedDataset(X_src, Y_src, mask_rate=0.15, is_train=True)
        source_loader = DataLoader(pretrain_dataset, batch_size=batch_size, shuffle=True)
        ...
        finetune_dataset = InterpolationMaskedDataset(X_train, Y_train, mask_rate=0.25, is_train=True)
        target_loader = DataLoader(finetune_dataset, batch_size=batch_size, shuffle=True)
...
ext_summary.to_csv('5_fold_new_transfer_results/mask_external_val/pinn_val_head.csv', index=False)
