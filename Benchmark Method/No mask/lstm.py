...
for fold_num, (train_idx, test_idx) in enumerate(kf.split(subjects), 1):
    for start_day in START_DAYS:
        ...
        input_size = len(microbe_cols)
        source_loader = DataLoader(
        MicrobiomeDataset(src_train[0], src_train[1]),batch_size=batch_size, shuffle=True
        )
        ...
        target_loader = DataLoader(
            MicrobiomeDataset(tgt_train[0], tgt_train[1]),
            batch_size=batch_size, shuffle=True
        )

        _,test_tuple, _, _ = load_train_test_data(test_data, test_data, known_steps)
        test_loader = DataLoader(
            MicrobiomeDataset(test_tuple[0], test_tuple[1]),
...
ext_summary.to_csv('5_fold_new_transfer_results/external_val/lstm_val_head.csv', index=False)

    # model_save_path = f"comparison_model/model/lstm_{fold_num}_transfer_lstm_model.pth"
    # torch.save(model.state_dict(), model_save_path)
    
#     test_subject_ids, test_times = test_tuple[2], test_tuple[3]
#     results = predict_and_save(model, test_loader, microbe_cols, test_subject_ids, test_times, save_path=model_save_path, fold_num=fold_num, results=results)
# results_df = pd.DataFrame(results)
# results_df.to_csv("comparison_model/results/hip_to_face/lstm_hip-to-face_result.csv", index=False)
