...
# os.makedirs("elasticnet/abundance", exist_ok=True)
...
for fold_num, (train_idx, test_idx) in enumerate(kf.split(subjects), 1):
    for start_day in START_DAYS:
        ...
        # print(X_src.shape, Y_src.shape, X_tgt.shape, Y_tgt.shape, X_test.shape, Y_test.shape)
        # train_data_tuple, test_data_tuple, input_size, microbe_columns = load_train_test_data(
        #     train_data, test_data, known_steps=known_steps)
...
ext_summary.to_csv('5_fold_new_transfer_results/external_val/ElasticNet_val_body.csv', index=False)
# results_df = pd.DataFrame(results)
# results_df.to_csv("comparison_model/results/face_to_hip/elasticnet_face_to_hip_result.csv", index=False)
