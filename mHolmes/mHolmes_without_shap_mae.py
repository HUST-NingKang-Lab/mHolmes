...
# full_hip_df = pd.read_csv('/home/hanjin/projects/my_project/Human cadaver tansfer learning/reverse_face_selected.csv')
# full_face_df = pd.read_csv('/home/hanjin/projects/my_project/Human cadaver tansfer learning/reverse_hip_selected.csv')
full_hip_df = pd.read_csv('/home/hanjin/projects/my_project/Human cadaver tansfer learning/reverse_face_keytaxa.csv')
full_face_df = pd.read_csv('/home/hanjin/projects/my_project/Human cadaver tansfer learning/reverse_hip_keytaxa.csv')
...
subjects = full_hip_df['ID'].unique()
kf = KFold(n_splits=5, shuffle=True, random_state=42)
all_results = []
ext_results = []  # 收集外部验证结果
...
for fold_idx, (train_idx, test_idx) in enumerate(kf.split(subjects), 1):
    for start_day in START_DAYS:
        if start_day + WINDOW_LEN - 1 > MAX_DAY:
            continue
        face_df = slice_by_window(full_hip_df, start_day, WINDOW_LEN)[0]
        hip_df = slice_by_window(full_face_df, start_day, WINDOW_LEN)[0]
        ...
        # mse, r2, corr = calculate_metrics(pred, labels)
        # all_results.append({'fold_num': fold_idx, 'method': 'mHolmes_without_transfer', 'mse': mse, 'r2': r2, 'corr': corr})
        mse_ext, r2_ext, corr_ext = eval_on_dataset(foundational_model, val_body_dataset)
        ext_results.append({'fold': fold_idx, 'start_day': start_day, 'mse': mse_ext, 'r2': r2_ext, 'corr': corr_ext})
        # # bc = bray_curtis_sum(pred, labels)
        # # 新增：外部验证，使用 val_head_dataset
        # mse_ext, r2_ext, corr_ext = eval_on_dataset(foundational_model, val_head_dataset)
...
ext_summary.to_csv('5_fold_new_transfer_results/external_val/mHolmes-without-transfer_val_body.csv', index=False)
# 在循环后保存外部验证结果
# pd.DataFrame(ext_results).to_csv('/home/hanjin/projects/my_project/Human cadaver tansfer learning/5_fold_new_transfer_results/external_val/mHolmes-without-transfer_val_head.csv', index=False)
# 修正 all_results 保存语法
# pd.DataFrame(all_results).to_csv('/home/hanjin/projects/my_project/Human cadaver tansfer learning/comparison_model/results/face_to_hip/mHolmes-without-transfer_face-to-hip_results.csv', index=False)
